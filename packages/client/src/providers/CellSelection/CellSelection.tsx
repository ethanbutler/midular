...
const CellSelectionContext = React.createContext<ReturnType<typeof useCellSelectionProvider>>(
  null as any
)
...
export function useCellSelectionProvider({
  ...
  const getIsCoordAtStart = (coords: Coords) => start && (coords.x === start.x) && (coords.y === start.y)

  /** Returns true if a provided set of coordinates is the ending point. */
  const getIsCoordAtEnd = (coords: Coords) => end && (coords.x === end.x) && (coords.y === end.y)

  /** Returns true if a provided set of coordinates is *either* the starting or ending point. */
  const getIsCoordActive = (coords: Coords) => getIsCoordAtStart(coords) || getIsCoordAtEnd(coords)
  ...
  const isColliding = Boolean(start && end && detectCollision(convertCoordPairToDimensions(start, end)))
